...
import { Search, ShoppingBag, Star, SlidersHorizontal, X, Flame, Sparkles } from "lucide-react";
...
export default function Menu() {
  ...
  const handleAddToCart = (item: any) => {
    addItem({
      id: crypto.randomUUID(), // ✅ fixed from Date.now() + Math.random()
      menuItemId: item.id,
      name: item.name,
      price: item.price,
      ...
      <div className="bg-white border-b border-amber-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="mt-2 text-amber-600">Discover our handcrafted cereal milkshakes</p>
          </motion.div>
                ...
                >
                  ...
                  >
                    ...
                    <div className="absolute top-3 left-3 flex flex-col gap-1">
                      {item.popular && (
                        <div className="flex items-center gap-1 px-2 py-1 bg-amber-500 text-white text-xs font-bold rounded-full">
