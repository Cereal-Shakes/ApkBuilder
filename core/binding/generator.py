...
from core.binding.parser import parse_layout
from core.binding.resolver import bind_views
...
class GenerateViewBinding:
    ...
    def start(self):
        os.makedirs(self.project.view_binding_dir, exist_ok=True)
        
        if not self.project.package_name:
            raise Exception("-- attribute 'package' not found in AndroidManifest.xml, please, add")
        
        get_logger().info("-- Generating binding files")
        
        package_name = self.project.package_name + ".databinding"
        output_dir = os.path.join(self.project.view_binding_dir, package_name.replace(".", "/"))
